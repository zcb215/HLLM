...
def get_model(model_name):
    ...
    print(f"moudle_path:{module_path}")
    print(f"__name__:{__name__}")
    # moudle_path:REC.model.IDNet.hllm
    # input()
    # 它会将module_path和__name__转换为绝对模块名称。如果module_path是相对路径，则使用__name__的包路径来解析。
    if importlib.util.find_spec(module_path, __name__):
        print("import IDnet")
        model_module = importlib.import_module(module_path, __name__)
        print("finish import IDnet")
